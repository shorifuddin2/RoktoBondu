...
const CampaignDetails = () => {
  ...
  useEffect(() => {
    let res = campaign.find((p) => p.id === parseInt(param));
    setData(res);
    let campaignLayout = campaign.filter((p) => p.id < 4);
    setLayout(campaignLayout);
  }, []);

  const { title, date, img, author, descr } = data;

  return (
    <>
      <Navbar />
      <div className="lg:mx-[120px] md:[50px] my-12 mx-5">
        <h1 className="text-xl text-black border-[#212529] border-b pb-5">
          {title}
        </h1>
        ...
        <div className="grid lg:grid-cols-3 md:grid-cols-2 gap-8 mt-12 md:container mx-auto">
          {layout.map((layout, index) => (
            ...
            >
              ...
              <div className="p-8">
                <h2 className=" text-[18px] font-bold text-[#606060]">
                  {layout.title.slice(0, 50)}
                </h2>
                <h5 className="text-[14px] mt-2">
                  <span>{layout.author}</span> | <span>{layout.date}</span>
                </h5>
                <p className=" text-[#212529] leading-6 text-[14px] mt-3 pb-12">
                  {layout.desc.slice(0, 100)}
                </p>
              </div>
              <button
                className="border text-sm font-semibold p-2 mb-3
                     hover:bg-black rounded-full py1- text-white px-6 mx-8 bg-primary absolute bottom-3"
              >
                বিস্তারিত...
              </button>
            </div>
