...
const First_roktodan_Details = () => {
  const params = useParams();
  const [Roktototthyo_Data, setRoktototthyo_Data] = useState("");
  const [leatestData, setLeatestData] = useState([]);

  useEffect(() => {
    let res = data.find((e) => e.id === params.id);
    setRoktototthyo_Data(res);
    setLeatestData(data.filter((e) => e.id <= 2));
  }, []);

  console.log(Roktototthyo_Data);
  const {
    id,
    title,
    author,
...
export default First_roktodan_Details;
